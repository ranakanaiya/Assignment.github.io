...
function renden_business_thinkup_child_frontscripts() {

	wp_enqueue_style( 'thinkup-style', get_template_directory_uri() . '/style.css', array( 'thinkup-bootstrap' ) );
	wp_enqueue_style( 'renden-business-thinkup-style', get_stylesheet_uri(), array( 'thinkup-style' ), wp_get_theme()->get('Version') );

	// Add theme scripts
	wp_enqueue_script( 'renden-business-thinkup-frontend', get_stylesheet_directory_uri() . '/lib/scripts/main-frontend.js', array( 'jquery', 'thinkup-frontend' ), wp_get_theme()->get('Version'), 'true' );

}
add_action( 'wp_enqueue_scripts', 'renden_business_thinkup_child_frontscripts' );
...
function renden_business_thinkup_reduxvariables() { 

	$GLOBALS['thinkup_blog_style1layout'] = 'option2';

}
add_action( 'thinkup_hook_header', 'renden_business_thinkup_reduxvariables', 11 );
...
function renden_business_thinkup_updateoption_child_settings() {
...
add_action( 'init', 'renden_business_thinkup_updateoption_child_settings', 999 );